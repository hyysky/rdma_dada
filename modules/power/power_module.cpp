...
public:
    ...
    std::unique_ptr<CudaPowerExecutor> cuda_executor;
};

PowerModule::PowerModule() : impl_(new Impl) {}

PowerModule::~PowerModule() {}

const char* PowerModule::Name() const { return "power"; }

pipeline::StageStatus PowerModule::ConfigureHeader(
    ...
    if (!input_header.GetString("DATA_STAGE", &data_stage) ||
        ...
        return pipeline::StageStatus::Error(
            "power input DATA_STAGE must be BEAMFORMED");
    }
    if (!input_header.GetString("ORDER", &order) || order != "TFPB") {
        return pipeline::StageStatus::Error("power input ORDER must be TFPB");
    }
    if (!input_header.GetString("SAMPLE_FORMAT", &sample_format) ||
        sample_format != "CF32") {
        return pipeline::StageStatus::Error(
            "power input SAMPLE_FORMAT must be CF32");
    }
    ...
    if (!CheckedMultiply(impl_->nchan, impl_->npol, &elements_per_frame) ||
        ...
        !CheckedMultiply(impl_->nchan, impl_->npol, &elements_per_frame) ||
        !CheckedMultiply(elements_per_frame, impl_->nbeam,
                         &elements_per_frame) ||
        !CheckedMultiply(elements_per_frame, kFloat32Bytes,
                         &impl_->output_frame_bytes)) {
        return pipeline::StageStatus::Error("power frame geometry overflows");
    }

    bool has_output_byte_rate = false;
    std::uint64_t output_byte_rate = 0;
    if (input_header.Has("BYTES_PER_SECOND")) {
        std::uint64_t input_byte_rate = 0;
        if (!input_header.GetUint64("BYTES_PER_SECOND", &input_byte_rate) ||
            input_byte_rate == 0 || input_byte_rate % 2 != 0) {
            return pipeline::StageStatus::Error(
                "BYTES_PER_SECOND cannot be represented after CF32 to F32 "
                "conversion");
        }
        output_byte_rate = input_byte_rate / 2;
        has_output_byte_rate = true;
    }
    ...
    if (impl_->execution_backend == pipeline::ExecutionBackend::kHost &&
        ...
        return pipeline::StageStatus::Error(
            "CPU_REFERENCE power requires MEMORY=HOST or PINNED_HOST");
    }
    if (impl_->execution_backend == pipeline::ExecutionBackend::kCuda &&
        input_memory != "CUDA_DEVICE") {
        return pipeline::StageStatus::Error(
            "CUDA power requires MEMORY=CUDA_DEVICE");
    }

    if (impl_->execution_backend == pipeline::ExecutionBackend::kCuda) {
#if defined(RDMA_DADA_HAVE_CUDA)
        impl_->cuda_executor = CreateCudaPowerExecutor();
        const pipeline::StageStatus cuda_status =
            impl_->cuda_executor->Configure(impl_->cuda_device);
        if (!cuda_status.ok()) {
            impl_->cuda_executor.reset();
            return cuda_status;
        }
    ...
    *output_header = input_header;
    output_header->SetString("DATA_STAGE", "POWER");
    output_header->SetString("ORDER", "TFPB");
    output_header->SetString("SAMPLE_FORMAT", "F32");
    output_header->SetString("PRODUCTS", "POWER");
    ...
    if (has_output_byte_rate) {
        output_header->SetUint64("BYTES_PER_SECOND", output_byte_rate);
    }
    if (impl_->execution_backend == pipeline::ExecutionBackend::kCuda) {
...
pipeline::StageStatus PowerModule::ProcessBlock(
    const pipeline::InputBlock& input, pipeline::OutputBlock* output,
    const pipeline::BlockExecutionContext& context) {
    if (!impl_->configured) {
        return pipeline::StageStatus::Error("power module is not configured");
    }
    ...
    if (RangesOverlap(input.data, input.size, output->data,
                      required_output_bytes)) {
        return pipeline::StageStatus::Error(
            "power input and output blocks must not overlap");
    }
    const std::uint64_t element_count = input.size / kComplex32Bytes;
    ...
    if (impl_->execution_backend == pipeline::ExecutionBackend::kCuda) {
        ...
        if (input.location != pipeline::MemoryLocation::kCudaDevice ||
            ...
            return pipeline::StageStatus::Error(
                "CUDA power requires CUDA device input and output blocks");
        }
        if (!impl_->cuda_executor) {
            return pipeline::StageStatus::Error(
                "CUDA power executor is unavailable");
        }
        const pipeline::StageStatus cuda_status =
            impl_->cuda_executor->Process(
                input, output, element_count, context);
        ...
        for (std::uint64_t i = 0; i < element_count; ++i) {
            destination[i] = source[i].real * source[i].real +
...
pipeline::StageStatus PowerModule::Finish() {
