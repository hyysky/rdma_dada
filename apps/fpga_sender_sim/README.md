...
`fpga_sender_sim` 是 Project VDIF v1 的 UDP 测试发送端。Station 身份只来自 VDIF
header，不从源 IP/port 推断。schema v1 保留原有功能测试模式，由操作系统选择源地址和
端口；schema v2 显式绑定 source IP/port，一个进程模拟一个 Station；schema v3 在同一
固定 source IP/port 和同一 UDP socket 上模拟一组 `station_ids`。
...
- `PACED`（schema v2/v3）：所有物理 sender 使用共同的未来 `start_utc`，按
  `transmit.target_gbps` 的 UDP datagram payload 速率发送；schema v3 的
  `time.group_count` 是天文时间组数，总包数为 `group_count × station_ids.size()`；
- schema v3 对每个时间组发送本机全部 Station，下一组轮换首个 Station，避免固定
  Station 永远先到或后到；组内所有 Station 保持完全相同的 VDIF 时间 key；
...
schema v2/v3 的 `payload_mode=REPEAT_TEMPLATE` 在启动时为每个配置的 Station 生成一次
payload，后续 packet 只更新 32-byte VDIF header。这样 Station slice 仍可区分，同时
避免 payload 生成成为测速瓶颈。`DETERMINISTIC` 继续逐 group 使用上述公式。
...
进程结束时输出一行 JSON，包含 source/destination、Station ID 列表、逐 Station
scheduled/sent 闭合、target/actual payload Gbps、PPS、packet/byte/batch/retry/
failure/overrun 计数、实际发送模板的
